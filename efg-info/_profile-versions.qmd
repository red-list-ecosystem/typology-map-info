...
qry <- sprintf("SELECT map_code, map_version, status, map_type
...
Read more details about [older or alternative versions](/map-info/`r efg_code_`_history.qmd) of maps for this functional group.

```{r}
#| output: asis
maps_available |>
    filter(!status %in% "valid") |>
    mutate(mdtext = sprintf("- %s: %s (*code: %s*, version %s) \n", 
        map_type, status, map_code, map_version)) |>
    pull(mdtext) |> 
    cat()
```
