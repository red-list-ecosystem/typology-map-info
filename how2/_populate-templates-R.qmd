...
This instructions are only needed in the initial set up or when we need to make changes in the structure of the profiles or map info pages. 

If we need to update content (but not the template structure), then we need to remove files from the freeze directory and run:
